...
for benchmark in filelist:
    ...
    try:
        subprocess.call([program, "--instanceFile", benchmarkdir+benchmark, "--mac", "1"], stdout=outfile)
    finally:
        outfile.close()
    
    outfile2 = open(benchmark+".txt", "w")
    try:
        subprocess.call([program, "--instanceFile", benchmarkdir+benchmark, "--mac", "0"], stdout=outfile2)
    finally:
        outfile2.close()
